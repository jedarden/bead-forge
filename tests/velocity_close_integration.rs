...
fn test_velocity_update_on_close() {
    ...
    storage.close_issue(bead_id, "Test completed", "test").unwrap();
    
    // Verify the session was updated
    let (closed_at, duration): (Option<String>, Option<i64>) = storage.with_immediate_transaction(|tx| {
        Ok(tx.query_row(
    ...
    assert!(duration >= 590 && duration <= 610, "Duration should be ~600 seconds (10 minutes), got {}", duration);
    
    // Verify velocity_stats was updated
    let (model, harness, issue_type, sample_count, p50): (String, String, String, i64, Option<i64>) = storage.with_immediate_transaction(|tx| {
        Ok(tx.query_row(

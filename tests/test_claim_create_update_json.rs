...
fn get_string(json: &Value, field: &str) -> String {
    ...
    target
        ...
        .unwrap_or_else(|| {
            panic!(
                "Field '{}' is not a string or is missing in {}: {}",
                field,
...
fn test_claim_json_success() {
    ...
    assert!(
        data.get("reclaimed").is_some(),
        "should have reclaimed field"
    );
    assert_eq!(
        data["reclaimed"].as_u64().unwrap(),
        0,
...
fn test_claim_json_empty_queue() {
    ...
    assert_eq!(
        data.get("bead_id").and_then(|v| v.as_str()),
        None,
        "bead_id should not be present when empty"
    );
}
...
fn test_create_json_validation_error() {
    ...
    assert!(
        ok,
        "create with invalid type should succeed (no validation): {err}"
    );
...
fn test_create_json_with_special_characters() {
    ...
    let (out, err, ok) = run_bf(
        ...
        &[
            "create",
            "--title",
            "Bead with emoji 🎉 and quotes \"test\"",
            "--description",
            "Description with\nnewlines and\ttabs",
            "--json",
        ],
...
fn test_update_json_success() {
    ...
    assert!(
        data.get("updated").is_some() && data["updated"].as_bool() == Some(true),
...
fn test_update_json_non_existent_bead() {
    ...
    assert!(
        err.contains("not found") || err.contains("Bead") || err.contains("does not exist"),
...
fn test_update_json_clear_assignee() {
    ...
    assert!(
        is_cleared,
        "assignee should be cleared (empty or null), got: {:?}",
        assignee
    );
}
...
fn test_update_json_status_transition() {
    ...
    for status in ["in_progress", "blocked", "closed"] {
        let (out, err, ok) = run_bf(
            &workspace,
            &["update", &bead_id, "--status", status, "--json"],
        );
...
fn test_update_json_with_special_characters() {
    ...
    let (out, err, ok) = run_bf(
        ...
        &[
            ...
            &bead_id,
            "--description",
            "Description with emoji 🎉 and \"quotes\"\nand newlines",
            "--json",
        ],
    );
...
fn test_json_valid_for_mutation_commands() {
    ...
    // Test create
    let (create_out, err, ok) = run_bf(
        &workspace,
        &["create", "--title", "Validation test", "--json"],
    );
    assert!(ok, "create failed: {err}");
    ...
    // Test update
    let (update_out, err, ok) = run_bf(
        &workspace,
        &["update", &bead_id, "--description", "Test", "--json"],
    );
    assert!(ok, "update failed: {err}");

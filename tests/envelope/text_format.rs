...
fn stats_envelope_output_matches_no_envelope() {
    ...
    assert_eq!(
        ...
        "Text output should be same with and without --envelope"
    );
}
...
fn claim_envelope_outputs_plain_text() {
    ...
    let output = run_envelope_command_text(
        ws.path(),
...
fn claim_envelope_output_matches_no_envelope() {
    ...
    let with_envelope = run_envelope_command_text(
        ws.path(),
        &["claim", "--assignee", "worker", "--format", "text"],
    );
    let without_envelope = run_command_text(
        ws.path(),
        &["claim", "--assignee", "worker", "--format", "text"],
    );
...
fn claim_envelope_empty_workspace() {
    ...
    let output = run_envelope_command_text(
        ws.path(),
        &["claim", "--assignee", "empty-worker", "--format", "text"],
    );
...
fn claim_envelope_outputs_bead_id() {
    ...
    let output = run_envelope_command_text(
        ws.path(),
        &["claim", "--assignee", "test-assignee", "--format", "text"],
    );
...
fn claim_envelope_structure_consistency() {
    ...
    let output1 = run_envelope_command_text(
        ws.path(),
        &["claim", "--assignee", "worker", "--format", "text"],
    );
    ...
    create_bead(ws.path(), "second bead");
    let output2 = run_envelope_command_text(
        ws.path(),
        &["claim", "--assignee", "worker", "--format", "text"],
    );
...
fn show_envelope_output_matches_no_envelope() {
    ...
    let with_envelope =
        run_envelope_command_text(ws.path(), &["show", "--format", "text", &bead_id]);

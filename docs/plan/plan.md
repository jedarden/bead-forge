...
**Known bug (currently live, blocking downstream consumers — fix tracked as bf-4njoy, children bf-51xnn and bf-24hp4):** `src/cli/mod.rs`'s `cmd_create` and `cmd_update` both call `validate_assignee()` (`src/validation.rs`) *before* reaching the correct storage-layer logic above, and `validate_assignee()` still rejects empty/whitespace-only values with `Error: Assignee cannot be empty or whitespace-only`. This means the correct clear-on-empty behavior is currently unreachable via the CLI — `bf update <id> --assignee ""` fails outright instead of clearing the field.

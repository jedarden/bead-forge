#!/bin/bash
# Test Epic Bead Type Functionality - bf-1rnkr
...
EPIC_DEPS=$(/home/coding/bead-forge/target/release/bf dep list "$EPIC_ID" | wc -l)
if [ "$EPIC_DEPS" != "3" ]; then
...
# Use grep to filter valid JSON lines and count them
TASK_COUNT=$(/home/coding/bead-forge/target/release/bf list --type task --json | grep -c '{' || true)
EPIC_COUNT=$(/home/coding/bead-forge/target/release/bf list --type epic --json | grep -c '{' || true)
BUG_COUNT=$(/home/coding/bead-forge/target/release/bf list --type bug --json | grep -c '{' || true)
...
# child1 should have only 1 dependency (the blocking dependency on BLOCKER)
# The parent-child relationship is stored on the epic (epic depends on child), not on the child
CHILD1_DEPS=$(/home/coding/bead-forge/target/release/bf dep list "$CHILD1" | grep -c "depends on" || true)
if [ "$CHILD1_DEPS" != "1" ]; then
  echo "FAIL: Expected 1 dependency for child1, got $CHILD1_DEPS"
  /home/coding/bead-forge/target/release/bf dep list "$CHILD1"
  exit 1
fi
...
CLOSED_CHILDREN=$(/home/coding/bead-forge/target/release/bf list --type task --status closed --json | grep -c '{' || true)
...
ALL_CHILDREN_CLOSED=$(/home/coding/bead-forge/target/release/bf list --type task --status closed --json | grep -c '{' || true)
ALL_CHILDREN_CLOSED=$((ALL_CHILDREN_CLOSED + $(/home/coding/bead-forge/target/release/bf list --type bug --status closed --json | grep -c '{' || true)))
...
ALL_EPICS_COUNT=$(/home/coding/bead-forge/target/release/bf list --type epic --json | grep -c '{' || true)
...
echo "--- Open epics ---"
OPEN_EPICS=$(/home/coding/bead-forge/target/release/bf list --type epic --status open --json | grep -c '{' || echo 0)
echo "Open epics: $OPEN_EPICS"

echo "--- Closed epics ---"
CLOSED_EPICS=$(/home/coding/bead-forge/target/release/bf list --type epic --status closed --json | grep -c '{' || echo 0)
...
# Get the most recently created beads by matching their titles
FEATURE_EPIC=$(/home/coding/bead-forge/target/release/bf list --type epic --status open --json | jq -r 'select(.title == "Epic: Feature Toggle System") | .id')
FEATURE_CHILD=$(/home/coding/bead-forge/target/release/bf list --type feature --status open --json | jq -r 'select(.title == "Add toggle flags") | .id')
CHORE_CHILD=$(/home/coding/bead-forge/target/release/bf list --type chore --status open --json | jq -r 'select(.title == "Update documentation") | .id')

echo "Selected epic: $FEATURE_EPIC, feature: $FEATURE_CHILD, chore: $CHORE_CHILD"
...
# Verify child types by checking the children directly using bf show
FEATURE_CHILD_TYPE=$(/home/coding/bead-forge/target/release/bf show "$FEATURE_CHILD" --json | jq -r 'if type == "array" then .[0].issue_type else .issue_type end')
CHORE_CHILD_TYPE=$(/home/coding/bead-forge/target/release/bf show "$CHORE_CHILD" --json | jq -r 'if type == "array" then .[0].issue_type else .issue_type end')
CHILD_TYPES="$FEATURE_CHILD_TYPE $CHORE_CHILD_TYPE"
